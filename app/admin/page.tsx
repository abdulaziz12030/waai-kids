...
import AdminTaskResetButton from "./AdminTaskResetButton";
...
export default function AdminPage() {
  ...
  async function runAction(id:string, action:() => Promise<{ error: { message:string }|null }>, message:string) {
    ...
    setBusyId("");
    if (result.error) {
      setError("لم تكتمل العملية. تحقق من الصلاحيات ثم حاول مرة أخرى.");
      return;
    }
    setSuccess(message);
    await loadDashboard();
  ...
  async function resetAllGifts() {
    if (!supabase) return;
    if (!window.confirm("سيتم حذف جميع الهدايا المرسلة وسجل إشعاراتها نهائيًا وتصفير عدد الهدايا. لن تُحذف أنواع الهدايا من المتجر، ولن يُمس رصيد الكوينز المشترى. متابعة؟")) return;
    const confirmationPhrase = window.prompt("اكتب العبارة التالية حرفيًا:\nتصفير الهدايا", "");
    if (confirmationPhrase !== "تصفير الهدايا") {
      setError("لم يتم التنفيذ لأن عبارة التأكيد غير صحيحة.");
      return;
    }
    const reason = window.prompt("سبب تصفير الهدايا:", "تصفير بيانات النسخة التجريبية") || "إجراء إداري";
    ...
    <main className={styles.shell}>
      <div className={styles.console}>
        <header className={styles.topbar}>
          ...
          <div className={styles.topActions}><Link className={styles.secondaryButton} href="/dashboard">لوحة الأسرة</Link><button className={styles.dangerButton} type="button" onClick={() => void signOut()}>خروج</button></div>
        </header>
