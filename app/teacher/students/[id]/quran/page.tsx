...
function formatDate(value: string | null) {
  if (!value) return "—";
  return new Intl.DateTimeFormat("ar-SA-u-ca-gregory", { day: "numeric", month: "long", year: "numeric" }).format(new Date(`${value}T00:00:00`));
}
...
export default function TeacherStudentQuranPage() {
  ...
  const selectedSurah = useMemo(() => quranSurahs.find((item) => item.number === Number(surahNumber)) || quranSurahs[0], [surahNumber]);
  const manualSurah = useMemo(() => quranSurahs.find((item) => item.number === Number(manualSurahNumber)) || quranSurahs[0], [manualSurahNumber]);
  ...
  return (
    <main className="quran-program-page teacher-quran-management-page">
      ...
      <section className="quran-program-hero">
        <div className="quran-hero-copy"><span className="section-label">👨‍🏫 الإدارة المهنية</span><h1>برنامج حفظ {studentName}</h1><p>{familyName} · أنشئ الخطة، تابع الورد اليومي، وقيّم التسميع من مركز المعلم.</p></div>
      ...
      <section className="quran-plans-card quran-full-card">
        <div className="quran-card-head"><div><span className="section-label">الجدول اليومي</span><h2>مقاطع الخطة المختارة</h2></div><span>{segments.length}</span></div>
        {segments.length === 0 ? <div className="quran-empty-state"><span>📖</span><h3>لا توجد مقاطع بعد</h3></div> : <div className="quran-segments-grid">{segments.map((segment) => <article className={`quran-segment-card status-${segment.status}`} key={segment.id}><div className="quran-plan-item-head"><div><span className="quran-plan-status">{statusLabels[segment.status] || segment.status}</span><h3>{segment.portion_label}</h3>{segment.scheduled_date && <small>📅 {formatDate(segment.scheduled_date)}</small>}</div><strong>{segment.achievement_points} ⭐</strong></div>{segment.readable_text && <p className="quran-readable-text">{segment.readable_text}</p>}{segment.notes && <p className="quran-segment-note">ملاحظة: {segment.notes}</p>}<div className="teacher-segment-actions">{["memorized", "recited"].includes(segment.status) && <Link href="/quran/reviews">فتح في مركز التسميع</Link>}<button type="button" disabled={busyId === segment.id || segment.status === "mastered"} onClick={() => deleteSegment(segment)}>{segment.status === "mastered" ? "مقطع معتمد" : busyId === segment.id ? "جارٍ الحذف..." : "حذف المقطع"}</button></div></article>)}</div>}
      </section>
    </main>
  );
}
