...
export default function MemorizationProgramCard(props: Props) {
  ...
  const selected = activeSegment?.plan_id === group.plan.id ? activeSegment : group.current;
  const upcoming = group.segments.filter((item) => ["assigned", "needs_revision"].includes(item.status) && item.id !== selected?.id);
  const waiting = group.segments.filter((item) => ["memorized", "recited"].includes(item.status) && item.id !== selected?.id);
  const mastered = group.segments.filter((item) => item.status === "mastered" && item.id !== selected?.id).reverse();
    ...
    return (
      <article className={`child-quran-queue-item ${isSelected ? "is-selected" : ""}`} key={segment.id}>
        <div className="queue-day-badge">
          <strong>{segment.day_number ? `المقطع ${segment.day_number}` : "مقطع"}</strong>
          <small>{segment.scheduled_date === today ? "اليوم" : isPast ? "مستحق" : formatDate(segment.scheduled_date)}</small>
        </div>
  ...
  return (
    <article className={`child-quran-program-card ${props.isOpen ? "is-open" : ""}`}>
      <button
        className="child-quran-program-toggle"
        type="button"
        aria-expanded={props.isOpen}
        disabled={Boolean(props.recordingSegmentId) && !props.isOpen}
        onClick={props.onToggle}
      >
        <span className="program-number">{props.index + 1}</span>
        <span className="program-main-copy">
          <small>{isMatn ? group.plan.subject_category || "العلوم الدينية" : "برنامج حفظ القرآن"}</small>
          <strong>{group.plan.title}</strong>
          <span>
            {group.plan.daily_target} {isMatn ? "أبيات في المقطع" : "آيات يوميًا"}
            {group.plan.duration_days ? ` · ${group.plan.duration_days} يومًا` : ""}
            {group.plan.due_date ? ` · النهاية ${formatDate(group.plan.due_date)}` : ""}
          </span>
        </span>
        <span className="program-progress-copy">
          <b>{group.progress}%</b><i><em style={{ width: `${group.progress}%` }} /></i>
      ...
      {props.isOpen && (
        <div className="child-quran-program-body">
          {group.segments.length === 0 ? (
            <div className="child-friendly-empty compact-empty"><span>🌤️</span><strong>لا توجد مقاطع في هذا البرنامج</strong></div>
          ) : (
            <>
              {isMatn ? (
                <MatnProgramReader
                  ...
                  onSelectCurrent={() => group.current && props.onSelectSegment(group.current.id)}
                />
              ) : selected && (
              ...
              <div className="child-quran-queues program-segment-queues">
                <details className="child-quran-queue-fold">
                  <summary><div><span className="queue-summary-icon">🗓️</span><div><strong>المقاطع الحالية والقادمة</strong><small>اختر أي مقطع لفتحه.</small></div></div><b>{upcoming.length}</b></summary>
                  <div className="child-quran-queue-body">
                    {upcoming.length ? upcoming.map((item) => compactSegment(item, item.scheduled_date && item.scheduled_date > today ? "فتح والبدء مبكرًا" : "فتح المقطع")) : <p className="queue-empty-message">لا توجد مقاطع أخرى حاليًا.</p>}
                  </div>
                </details>
                {waiting.length > 0 && (
                  <details className="child-quran-queue-fold waiting-fold">
                    <summary><div><span className="queue-summary-icon">⏳</span><div><strong>بانتظار المراجعة</strong></div></div><b>{waiting.length}</b></summary>
                    <div className="child-quran-queue-body">{waiting.map((item) => compactSegment(item, "عرض المقطع"))}</div>
                  </details>
                )}
                {mastered.length > 0 && (
                  <details className="child-quran-queue-fold mastered-fold">
                    <summary><div><span className="queue-summary-icon">🏆</span><div><strong>المقاطع المتقنة</strong></div></div><b>{mastered.length}</b></summary>
                    <div className="child-quran-queue-body">{mastered.map((item) => compactSegment(item, "عرض الإنجاز"))}</div>
                  </details>
                )}
              </div>
            </>
