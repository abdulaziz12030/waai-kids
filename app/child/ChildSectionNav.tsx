  ...
  return (
    <>
      ...
      <nav className="child-section-nav child-unified-nav" aria-label="التنقل الرئيسي لحساب الطفل">
        ...
        <button className={itemClass("tasks")} type="button" onClick={() => openDashboardSection("tasks")}><span>✅</span><small>مهامي</small></button>
        <Link className={itemClass("quran")} href="/child/quran"><span>📖</span><small>حفظي</small></Link>
        <button className={itemClass("progress")} type="button" onClick={() => openDashboardSection("progress")}><span>🚀</span><small>تقدمي</small></button>
        <button className={itemClass("goals")} type="button" onClick={() => openDashboardSection("goals")}><span>🎯</span><small>أهدافي</small></button>
        <Link className={itemClass("gifts")} href="/child/gifts"><span>🎁</span><small>هداياي</small></Link>
        <Link className={itemClass("notifications")} href="/child/notifications"><span>🔔</span><small>إشعاراتي</small></Link>
      </nav>
    </>
  );
}
