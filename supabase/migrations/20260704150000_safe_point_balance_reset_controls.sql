...
set search_path to 'public', 'auth'
as $function$
  select public.reset_student_point_balance(
    p_student_id,
    'achievement',
    'تصفير نقاط الإنجاز من ملف الطفل'
  );
$function$;

create or replace function public.parent_zero_student_reward_points(p_student_id uuid)
returns jsonb
language sql
security definer
set search_path to 'public', 'auth'
as $function$
  select public.reset_student_point_balance(
    p_student_id,
    'reward',
    'تصفير نقاط المكافآت من ملف الطفل'
  );
$function$;

create or replace function public.parent_zero_student_points(p_student_id uuid)
returns jsonb
language sql
security definer
set search_path to 'public', 'auth'
as $function$
  select public.reset_student_point_balance(
    p_student_id,
    'all',
    'تصفير الإنجاز والمكافآت من ملف الطفل'
  );
$function$;

create or replace function public.admin_reset_student_points(
  p_student_id uuid,
  p_point_type text,
  p_reason text default null
)
...
begin
  ...
  return public.reset_student_point_balance(
    p_student_id,
    p_point_type,
    coalesce(nullif(trim(p_reason), ''), 'تصفير إداري للرصيد')
  );
end;
$function$;

revoke all on function public.reset_student_point_balance(uuid,text,text) from public, anon;
revoke all on function public.parent_zero_student_achievement_points(uuid) from public, anon;
revoke all on function public.parent_zero_student_reward_points(uuid) from public, anon;
revoke all on function public.parent_zero_student_points(uuid) from public, anon;
revoke all on function public.admin_reset_student_points(uuid,text,text) from public, anon;
...
grant execute on function public.parent_zero_student_reward_points(uuid) to authenticated;
grant execute on function public.parent_zero_student_points(uuid) to authenticated;
grant execute on function public.admin_reset_student_points(uuid,text,text) to authenticated;
