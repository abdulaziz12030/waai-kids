...
begin
  ...
  return jsonb_build_object(
    'admin', (
      select jsonb_build_object('role', a.role, 'user_id', a.user_id)
      from public.platform_admins a where a.user_id = auth.uid()
    ),
    ...
    'organizations', coalesce((
      ...
      from (
        select o.id, o.name, o.type::text as type, o.family_title, o.city, o.family_code,
               o.created_at, u.email as owner_email,
    ...
    'subscriptions', coalesce((
      ...
      from (
        ...
        join public.organizations o on o.id=sub.organization_id
    ...
    'recent_gifts', coalesce((
      select jsonb_agg(to_jsonb(x) order by x.gifted_at desc)
      from (
        select cg.id, cg.student_id, s.full_name as student_name, o.name as organization_name,
               gc.name as gift_name, cg.achievement_title, cg.sender_name, cg.status, cg.coin_cost,
               cg.gifted_at, cg.opened_at
        from public.child_gifts cg
        join public.students s on s.id=cg.student_id
